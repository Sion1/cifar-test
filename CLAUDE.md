...
| **C** +AA  | autoaug   | sgd   | cosine    | 002 | 0.9519 | success (1-seed) → 008 s=4078 = 0.9529, **2-seed mean = 0.9524** (σ ≈ 0.0007) | **0.9524** |
...
- None yet — phase 1 in progress. Provisional 1-seed leader: **iter 002 Cell C
  ...
  **Cell C 2-seed-hardened** at iter 008 = **0.9529 (best=final, ep58)**
  with seed=4078; Cell C 2-seed mean = **0.9524**, peak-to-peak spread
  just **0.10 pp** — Cell C is highly seed-stable and clears stretch
  ≥ 0.95 on both seeds. **Crowning is gated** on iter 009 (Cell B
  seed=4078, already completed, awaiting analysis): only the matched
  B-side number can confirm whether the +0.42 pp single-seed C−B gap
  survives 2-seed averaging.
